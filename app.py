import pandas as pd
import streamlit as st
import plotly.graph_objects as go

# Load data
df = pd.read_excel('AVG FARE As at 29Dec Snap.xlsx', sheet_name='AVG_FARE', header=3)

# Dropdown options
from_city_options = df['FROM_CITY'].unique()
month_options = df['Month'].unique()

# Streamlit widgets
FROM_CITY = st.selectbox('Select From City:', from_city_options)
to_city_options = df[df['FROM_CITY'] == FROM_CITY]['TO_CITY'].unique()
TO_CITY = st.selectbox('Select To City:', to_city_options)
Month = st.selectbox('Select Month:', month_options)

def avg_fare(FROM_CITY, TO_CITY, Month):
    # Filter data
    month_data = df[df["Month"] == Month]
    month_data = month_data.drop(['SEG KEY', 'SEG KEY LY', 'MonthM_LY', 'Year', 'Month', 'Revenue_USD ', 
                                  'PAX_TY', 'PAX LY', "29Dec'24", "29Dec'23", "29Dec'24.", "29Dec'23.", 
                                  'Revenue_USD LY'], axis=1)
    row = month_data[(month_data["FROM_CITY"] == FROM_CITY) & (month_data["TO_CITY"] == TO_CITY)]

    if row.empty:
        st.warning(f"No data found for {FROM_CITY} to {TO_CITY}.")
        return

    # Prepare data
    xorder = ['03-Nov', '10-Nov', '17-Nov', '24-Nov', '01-Dec', '08-Dec', '15-Dec', '22-Dec', '29-Dec']
    row_1 = row.iloc[0, 4:21][::2]
    row_2 = row.iloc[0, 5:22][::2]
    row_1_reversed, row_2_reversed = row_1.iloc[::-1], row_2.iloc[::-1]
    difference = row_2_reversed.values - row_1_reversed.values

    # Fare Average Graph
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=xorder, y=row_1_reversed.values, mode='lines+markers', name="Fare Average - TY"))
    fig1.add_trace(go.Scatter(x=xorder, y=row_2_reversed.values, mode='lines+markers', name="Fare Average - LY"))
    fig1.add_trace(go.Scatter(x=xorder, y=row_1_reversed.rolling(window=3).mean(), mode='lines', name="MA - TY", line=dict(dash='dot', color='orange')))
    fig1.add_trace(go.Scatter(x=xorder, y=row_2_reversed.rolling(window=3).mean(), mode='lines', name="MA - LY", line=dict(dash='dot', color='yellow')))
    fig1.add_hline(y=row.iloc[0, 3], line=dict(color='red', dash='dash'), annotation_text="Last Year Actual Avg Fare")
    fig1.update_layout(title=f"Behavior of Avg Fare - {FROM_CITY} to {TO_CITY}", xaxis_title="Snap Dates", yaxis_title="Avg Fare (USD)", legend_title="Legend", template="plotly_dark", xaxis=dict(tickvals=xorder), hovermode="x unified")
    st.plotly_chart(fig1, use_container_width=True)

    # Difference Graph
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=xorder, y=difference, mode='lines+markers', name="Difference (LY - TY)", line=dict(dash='dot')))
    fig2.add_hline(y=0, line=dict(color='blue', dash='dash'), annotation_text="Zero Line")
    fig2.update_layout(title="Difference (LY - TY)", xaxis_title="Snap Dates", yaxis_title="Difference in Fare (USD)", legend_title="Legend", template="plotly_dark", xaxis=dict(tickvals=xorder), hovermode="x unified")
    st.plotly_chart(fig2, use_container_width=True)

    # Fare Data Table
    fare_data = pd.DataFrame({
        'Date': xorder,
        'Fare Average - TY': row_1_reversed.values,
        'Fare Average - LY': row_2_reversed.values,
        'Difference (LY - TY)': difference
    })
    st.subheader("Fare Data Table")
    st.dataframe(fare_data)  # Interactive table

def pax(FROM_CITY, TO_CITY, Month):
    # Filter data
    month_data = df[df["Month"] == Month]
    month_data = month_data.drop(['SEG KEY', 'SEG KEY LY', 'MonthM_LY', 'Year', 'Month', 'Revenue_USD ',
                                  "29Dec'24", "29Dec'23", "29Dec'24.", "29Dec'23.", 'Revenue_USD LY'], axis=1)
    row = month_data[(month_data["FROM_CITY"] == FROM_CITY) & (month_data["TO_CITY"] == TO_CITY)]

    if row.empty:
        st.warning(f"No data found for {FROM_CITY} to {TO_CITY}.")
        return

    # Prepare data
    xorder = ['03-Nov', '10-Nov', '17-Nov', '24-Nov', '01-Dec', '08-Dec', '15-Dec', '22-Dec', '29-Dec']
    row_1 = row.iloc[0, 6:24][::2]
    row_2 = row.iloc[0, 7:25][::2]
    row_1_reversed, row_2_reversed = row_1.iloc[::-1], row_2.iloc[::-1]
    difference = row_2_reversed.values - row_1_reversed.values

    # Pax Graph
    fig3 = go.Figure()
    fig3.add_trace(go.Scatter(x=xorder, y=row_1_reversed.values, mode='lines+markers', name="Pax - TY"))
    fig3.add_trace(go.Scatter(x=xorder, y=row_2_reversed.values, mode='lines+markers', name="Pax - LY"))
    fig3.update_layout(title=f"Behavior of Pax - {FROM_CITY} to {TO_CITY}", xaxis_title="Snap Dates", yaxis_title="Pax", legend_title="Legend", template="plotly_dark", xaxis=dict(tickvals=xorder), hovermode="x unified")
    st.plotly_chart(fig3, use_container_width=True)

    # Difference Graph
    fig4 = go.Figure()
    fig4.add_trace(go.Scatter(x=xorder, y=difference, mode='lines+markers', name="Difference (LY - TY)", line=dict(dash='dot')))
    fig4.add_hline(y=0, line=dict(color='blue', dash='dash'), annotation_text="Zero Line")
    fig4.update_layout(title="Difference (LY - TY)", xaxis_title="Snap Dates", yaxis_title="Difference in Pax", legend_title="Legend", template="plotly_dark", xaxis=dict(tickvals=xorder), hovermode="x unified")
    st.plotly_chart(fig4, use_container_width=True)

    # Pax Data Table
    pax_data = pd.DataFrame({
        'Date': xorder,
        'Pax - TY': row_1_reversed.values,
        'Pax - LY': row_2_reversed.values,
        'Difference (LY - TY)': difference
    })
    st.subheader("Pax Data Table")
    st.dataframe(pax_data)  # Interactive table

# Run functions
avg_fare(FROM_CITY, TO_CITY, Month)
pax(FROM_CITY, TO_CITY, Month)
